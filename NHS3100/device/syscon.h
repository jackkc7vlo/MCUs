 ...
 * 
 * System configuration controller (SYSCON)
 */
...
typedef struct
{
    ...
    volatile uint32_t reserved1[2];     //!< Reserved                                 offset 0x028 - 0x02C
    volatile uint32_t sysrststat;       //!< System reset status register             offset 0x030
    volatile uint32_t reserved2[19];    //!< Reserved                                 offset 0x034 - 0x07C
    volatile uint32_t ahbclkctrl;       //!< AHB clock control register               offset 0x080
    volatile uint32_t reserved3[4];     //!< Reserved                                 offset 0x084 - 0x094
    volatile uint32_t sspclkdiv;        //!< SSP/SSI clock divider register           offset 0x094
    volatile uint32_t reserved4[14];    //!< Reserved                                 offset 0x098 - 0x0D0
    ...
    volatile uint32_t reserved5[3];     //!< Reserved                                 offset 0x0DC - 0x0F4
    volatile uint32_t clkouten;         //!< CLKOUT enable register                   offset 0x0E8
    volatile uint32_t reserved6[26];    //!< Reserved                                 offset 0x0EC - 0x154
    volatile uint32_t systickcal;       //!< SysTick calibration register             offset 0x154
    volatile uint32_t reserved7[42];    //!< Reserved                                 offset 0x158 - 0x200
    ...
    volatile uint32_t reserved8[9];     //!< Reserved                                 offset 0x210 - 0X230
 ...
 *
 * @details This register allows software to reset some peripherals. Writing a logic 0 to the bits resets
 * the corresponding peripheral. Writing a logic 1 de-asserts the reset.
 * Remark: Before accessing the SPI and I2C peripherals, write a logic 1 to this register.
 * This action ensures that the reset signals to the SPI and I2C are de-asserted.
 */
 ...
 *
 * @details The system clock control register controls the analog part of the internal 8 MHz oscillator
 * as well as the system clock divider.
 * For changes to the system clock divider to take effect, first write a logic 0 to the
 * SYSCLKUEN register, then write a logic 1 to SYSCLKUEN.
 * See Section 4.5.3 "System clock divider" for more information.
 */
 ...
 *
 * @details This register determines the divider values for the watchdog clock wdt_clk. For the update
 * to take effect at the input of the watchdog timer, a 1-0-1 sequence has to be written to
