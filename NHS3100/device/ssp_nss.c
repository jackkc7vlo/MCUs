...
static void Chip_SSP_SetSSPClkDivider(NSS_SSP_T *pSSP, uint32_t div)
{
    if (pSSP == NSS_SSP0)
    {
        ...
        NSS_SYSCON->SSP0CLKDIV = (uint32_t)div;
    }
}
...
static uint32_t Chip_SSP_GetPCLKkRate(NSS_SSP_T *pSSP)
{
    ...
    if (pSSP == NSS_SSP0)
    {
        ...
        sspCLK = NSS_SYSCON->SSP0CLKDIV & 0xFF;
        if (sspCLK > 2)
        {
            sspCLK = sspCLK & 0xFE;
        }
...
uint32_t Chip_SSP_ReadFrames_Blocking(NSS_SSP_T *pSSP, uint8_t *buffer, uint32_t buffer_len)
{
    ...
    if (Chip_SSP_GetDataSize(pSSP) > SSP_BITS_8)
    {
        ...
        while (tx_cnt < buffer_len || rx_cnt < buffer_len)
        {
            ...
            if ((Chip_SSP_GetStatus(pSSP, SSP_STAT_TNF) == SET) && (tx_cnt < buffer_len))
            {
                Chip_SSP_SendFrame(pSSP, 0x0); /* just send dummy data */
                tx_cnt += 2;
            }
...
uint32_t Chip_SSP_GetBitRate(NSS_SSP_T *pSSP)
{
    ...
    uint32_t sysClk = (uint32_t)clock_hal_get_freq();
...
{
    NSS_SYSCON->SYSAHBCLKCTRL |= Chip_SSP_GetClockIndex(pSSP) & 0x1DBFFC;
    ...
    Chip_SSP_SetSSPClkDivider(pSSP, (uint32_t)(1 << ((NSS_SYSCON->SYSCLKCTRL >> 1) & 0x7)));

    NSS_SYSCON->PRESETCTRL |= (Chip_SSP_GetResetIndex(pSSP) & 0xf);
