/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief CMSIS Compatible EFM32PG22
                                                                               *startup file in C. Should be used with
...
void Default_Handler(void)
{
    ...
    vectactive                     = SCB->ICSR & SCB_ICSR_VECTACTIVE_Msk;
    last_default_handler_interrupt = vectactive;

    /* Convert exception number to IRQn number when applicable */
    if (vectactive >= 16UL)
    {
        last_default_handler_irqn = vectactive - 16UL;
    }
    else
    {
        last_default_handler_irqn = 0xFFFFFFFFUL;
    }

    /* Snapshot NVIC pending/active state for additional context */
    last_default_nvic_ispr0 = NVIC->ISPR[0];
    ...
    /* 19 = LETIMER0_IRQn, 21 = GPIO_ODD_IRQn, 22 = GPIO_EVEN_IRQn, etc. */
